...
Organizations deal with mixed document types (scanned, digital, forms) and need one pipeline that classifies, extracts, validates, and routes for human review when confidence is low.
...
1. Phase 1: Document classifier (scanned vs digital, type)
2. Phase 2: Unified parsing/OCR pipeline building on document-ai-yolo-ocr
3. Phase 3: Validation rules and confidence scoring
4. Phase 4: Human review queue for low-confidence documents

## Task Tracking

Work is broken into phase-tagged user stories tracked as GitHub Issues, not in this file. To see what's open:

    gh issue list --repo faheemkhaskheli9/intelligent-document-processing --state open --label type:user-story
...
```bash
docker build -t intelligent-document-processing .
docker run -p 8000:8000 intelligent-document-processing
```
